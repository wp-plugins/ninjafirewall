...
<div class="wrap">
	...
	<br /><br />
	That's cool and makes NinjaFirewall a true firewall. And probably one of the most powerful security applications for WordPress. But just like any firewall, if you misuse it, you can get into serious problems and crash your site...
	<br />
	<br />
...
<div class="wrap">
	...
	// PHP as CGI :
		...
		if ( $phpini ) {
			// fetch its content :
			$data = file_get_contents( ABSPATH . $phpini );
			...
			if ( $phpini_writable ) {
				echo '<li>I will add the following <font color="red">red lines</font> of code to your <code>' . ABSPATH . $phpini . '</code> file. All other lines, if any, are the actual content of the file&nbsp;:</li>';
				$button_title = 'Apply changes';
			} else {
				echo '<li>I cannot make any change to your system because it is read-only. Please download the <code>' . ABSPATH . $phpini . '</code> file with your FTP client, overwrite its content with the one below (the <font color="red">red lines</font> of code will be used by NinjaFirewall, and all other lines, if any, are the actual content of the file). Then re-upload it to your server&nbsp;:</li>';
				$button_title = 'Test configuration';
				$diy = 1;
			}
		} else {
			...
			if ( is_writable( ABSPATH ) ) {
				...
				echo '<li>I cannot make the required changes because your system is read-only. Using your FTP client, you need to create a PHP INI file inside your WordPress main directory. Usually, such a file is named <code>php.ini</code> but some hosting companies may use <code>php5.ini</code> or <code>.user.ini</code> files instead.
				<br />
				...
				<br /><br />
				Please add the following <font color="red">red lines</font> of code to that file and upload it into your WordPress root directory (<code>' . ABSPATH . '<font id="phpiniusertxt">php.ini</font></code>)&nbsp;:</li>';
				$diy = 1;
				$button_title = 'Test configuration';
				// default name :
				$phpini_user = 'php.ini';
		...
		echo '</pre>
		<img src="' . plugins_url( '/images/icon_warn_16.png', __FILE__ ) . '" border="0" height="16" width="16">&nbsp;Some hosting accounts may require a few minutes before a newly uploaded or modified PHP INI file is taken into consideration by PHP. This value is often 300 seconds/5 minutes by default ( see <a href="http://php.net/manual/en/configuration.file.per-user.php" target="_blank">user_ini.cache_ttl</a> ). If NinjaFirewall does not work after applying your changes, please wait 5 minutes and try again.
		<br />
		<br />';
		...
		if ( $htaccess ) {
			...
			if ( $htaccess_writable ) {
				echo '<li>I will add the following <font color="red">red lines</font> of code to your <code>' . ABSPATH . $htaccess . '</code> file. All other lines, if any, are the actual content of the file&nbsp;:</li>';
				$button_title = 'Apply changes';
			} else {
				echo '<li>I cannot make any change to your system because it is read-only. Please download the <code>' . ABSPATH . $htaccess . '</code> file with your FTP client, overwrite its content with the one below (the <font color="red">red lines</font> of code will be used by NinjaFirewall, and all other lines, if any, are the actual content of the file). Then re-upload it to your server&nbsp;:</li>';
				$button_title = 'Test configuration';
				$diy = 1;
			}
		} else {
			// There is no .htaccess, we need to create one :
			if ( is_writable( ABSPATH ) ) {
				echo '<li>I will create a <code>' . ABSPATH . '.htaccess</code> file and will add the following <font color="red">red lines</font> of code&nbsp;:</li>';
				$button_title = 'Apply changes';
			} else {
				echo '<li>I cannot make the required changes because your system is read-only. Please create a file named <code>.htaccess</code> ( see "<a href="http://codex.wordpress.org/Using_Permalinks#Creating_and_editing_.28.htaccess.29" target="_blank">Creating and editing .htaccess</a>" ), add the following <font color="red">red lines</font> of code and, using your FTP client, upload it into your WordPress root directory (<code>' . ABSPATH . '.htaccess</code>)&nbsp;:</li>';
				$diy = 1;
				$button_title = 'Test configuration';
			}
		}
...
function nfw_install_3() {
	...
	// We need to create a .htaccess :
		...
		$nfw_conf_arr['htaccess_path'] = ABSPATH;
	}
	...
	// User defined PHP INI ?
		...
		$nfw_conf_arr['phpini_path'] = ABSPATH;
	}
	...
	if ( $nfw_conf_arr['phpini'] ) {
		...
		if ( file_put_contents( $nfw_conf_arr['phpini_path']. $nfw_conf_arr['phpini'],
			...
			@chmod( ABSPATH . $nfw_conf_arr['phpini'], 0644 );
		}
	}
	echo '
<div class="wrap">
